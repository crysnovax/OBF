//#*******************premium obfuscating technology build by crysnovax verified**********************#//
const CORS_HEADERS = {
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    input[type="text"] { width:100%;padding:1rem;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);color:var(--text);font-family:'Inter',sans-serif;font-size:0.9rem;outline:none; }
...
<body>
  ...
  <div class="container">
    ...
    <div class="section" id="obfuscate">
      ...
      <div class="tab-content" id="tab-github">
        <div class="editor-card"><div class="editor-body"><input type="text" id="githubUrl" placeholder="https://github.com/username/repo/blob/main/file.js"><p style="margin-top:0.8rem;font-size:0.85rem;color:var(--text3)">Enter a public GitHub file URL to clone and obfuscate</p></div></div>
      </div>
  ...
  <script>
    ...
    function switchTab(tab) {
      ...
      for (var i = 0; i < tabs.length; i++) { if (tabs[i].textContent.toLowerCase().indexOf(tab) !== -1) tabs[i].classList.add('active'); }
    }
    function selectLevel(level) { selectedLevel = level; var cards = document.querySelectorAll('.option-card'); for (var i = 0; i < cards.length; i++) { cards[i].classList.remove('selected'); } var card = document.querySelector('[data-level="'+level+'"]'); if (card) card.classList.add('selected'); }
    ...
    async function obfuscate() {
      ...
      try {
        ...
        var statsHtml = '';
        statsHtml += '<div class="result-stat"><div class="result-stat-value">'+data.stats.originalSize+'</div><div class="result-stat-label">Original Size</div></div>';
        statsHtml += '<div class="result-stat"><div class="result-stat-value">'+data.stats.obfuscatedSize+'</div><div class="result-stat-label">Obfuscated Size</div></div>';
        statsHtml += '<div class="result-stat"><div class="result-stat-value" style="color:var(--warning)">'+data.stats.increase+'</div><div class="result-stat-label">Size Increase</div></div>';
        statsHtml += '<div class="result-stat"><div class="result-stat-value" style="color:var(--success)">'+data.stats.level.toUpperCase()+'</div><div class="result-stat-label">Protection</div></div>';
    ...
    function copyResult() { navigator.clipboard.writeText(currentResult).then(function() { showToast('📋 Copied!', 'success'); }); }
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === 'POST' && path === '/api/obfuscate') {
      try {
        ...
        if (!body.code) return errorResponse('No code provided');
    ...
    if (method === 'POST' && path === '/api/obfuscate/github') {
      try {
        ...
        if (!response.ok) return errorResponse('Failed to fetch from GitHub: ' + response.status);
