...
// Generate unique ID
function generateId() {
  return 'obf_' + Math.random().toString(36).substring(2, 10) + Date.now().toString(36);
}
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .gradient-text {
      background: linear-gradient(135deg, #6366f1 0%, #818cf8 30%, #a78bfa 60%, #6366f1 100%);
...
<body>
  ...
  <div class="loading-overlay" id="loading">
    ...
    <div class="loading-text">Obfuscating your code...</div>
  </div>
  ...
  <div class="container">
    ...
    <nav class="nav">
      <a href="/" class="nav-logo">
        ...
        <div class="logo-text">CRYSNOVA Obfuscator</div>
      </a>
      <div class="nav-links">
    ...
    <div class="hero">
      ...
      <h1 class="hero-title">
        <span class="gradient-text">Obfuscate</span><br>
        Your JavaScript
      </h1>
      <p class="hero-subtitle">
        Military-grade JavaScript obfuscation with anti-debug protection, 
        control flow flattening, and self-defending capabilities. 
        Protect your intellectual property in seconds.
      </p>
      ...
      <div class="hero-stats">
        <div class="hero-stat">
          ...
          <div class="hero-stat-label">Files Protected</div>
        </div>
        <div class="hero-stat">
          <div class="hero-stat-value">3</div>
          <div class="hero-stat-label">Protection Levels</div>
        </div>
        <div class="hero-stat">
    ...
    <div class="section" id="features">
      ...
      <div class="features-grid">
        <div class="feature-card">
          ...
          <div class="feature-title">Control Flow Flattening</div>
          <div class="feature-desc">Transforms code structure into complex, unreadable flow patterns that are nearly impossible to reverse engineer.</div>
        </div>
        <div class="feature-card">
          <div class="feature-icon">🔐</div>
          <div class="feature-title">String Encoding</div>
          <div class="feature-desc">Encrypts all string literals using advanced encoding algorithms to hide sensitive data and logic.</div>
        </div>
        <div class="feature-card">
          <div class="feature-icon">🛡️</div>
          <div class="feature-title">Self Defending</div>
          <div class="feature-desc">Code detects tampering attempts and triggers anti-debugging countermeasures automatically.</div>
        </div>
        <div class="feature-card">
          <div class="feature-icon">🔗</div>
          <div class="feature-title">GitHub Integration</div>
          <div class="feature-desc">Directly clone repositories from GitHub and obfuscate entire projects in one click.</div>
        </div>
        <div class="feature-card">
          <div class="feature-icon">🤖</div>
          <div class="feature-title">Bot API Access</div>
          <div class="feature-desc">Integrate with WhatsApp bot for on-the-go obfuscation via simple API endpoints.</div>
        </div>
        <div class="feature-card">
          <div class="feature-icon">⚡</div>
          <div class="feature-title">Lightning Fast</div>
          <div class="feature-desc">Powered by Cloudflare's global edge network for instant obfuscation worldwide.</div>
        </div>
      </div>
    </div>
    ...
    <div class="section" id="obfuscate">
      ...
      <div class="tab-content active" id="tab-paste">
        <div class="editor-card">
          <div class="editor-header">
            ...
            <div style="color:var(--text3);font-size:0.85rem">source.js</div>
          </div>
          <div class="editor-body">
            <textarea id="codeInput" placeholder="// Paste your JavaScript code here...&#10;&#10;const greeting = 'Hello World';&#10;console.log(greeting);"></textarea>
          </div>
        </div>
      </div>
      ...
      <div class="tab-content" id="tab-upload">
        <div class="upload-area" onclick="document.getElementById('fileInput').click()">
          ...
          <div class="upload-text">Drop your .js file here or click to browse</div>
          <div class="upload-hint">Supports .js and .mjs files up to 5MB</div>
        </div>
        ...
        <div class="editor-card">
          <div class="editor-body">
            <input type="text" id="githubUrl" placeholder="https://github.com/username/repo" 
              style="width:100%;padding:1rem;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);color:var(--text);font-family:'Inter',sans-serif;font-size:0.9rem;outline:none">
            <p style="margin-top:0.8rem;font-size:0.85rem;color:var(--text3)">Enter a public GitHub repository URL to clone and obfuscate</p>
          </div>
        </div>
      </div>
      ...
      <div class="options-grid" id="protectionLevels">
        <div class="option-card" onclick="selectLevel('light')" data-level="light">
          <div class="option-icon">🟢</div>
          <div class="option-title">Light Protection</div>
          <div class="option-desc">Variable renaming and whitespace removal. Fast execution, readable but protected.</div>
          <span class="option-level level-light">Light</span>
        </div>
        <div class="option-card selected" onclick="selectLevel('medium')" data-level="medium">
          <div class="option-icon">🟡</div>
          <div class="option-title">Medium Protection</div>
          <div class="option-desc">String encoding + control flow flattening. Strong protection with good performance.</div>
          <span class="option-level level-medium">Medium</span>
        </div>
        <div class="option-card" onclick="selectLevel('heavy')" data-level="heavy">
          <div class="option-icon">🔴</div>
          <div class="option-title">Heavy Protection</div>
          <div class="option-desc">Dead code injection + self-defending + anti-debug. Maximum security, larger output.</div>
          <span class="option-level level-heavy">Heavy</span>
        </div>
      </div>

      <button class="btn btn-primary btn-lg btn-block" onclick="obfuscate()" style="margin-top:1rem">
        🛡️ Obfuscate Now
      </button>
  ...
  <script>
    ...
    function switchTab(tab) {
      ...
      const tabMap = { paste: 'tab-paste', upload: 'tab-upload', github: 'tab-github' };
    ...
    async function obfuscate() {
      ...
      if (activeTab === 'tab-paste') {
        ...
        code = uploadedCode;
      } else if (activeTab === 'tab-github') {
        isGithub = true;
        githubUrl = document.getElementById('githubUrl').value;
      }

      if (!code && !isGithub) {
        showToast('Please provide code to obfuscate', 'error');
        return;
      }
      ...
      try {
        ...
        let body = { code, level: selectedLevel };

        if (isGithub) {
          endpoint = '/api/obfuscate/github';
          body = { url: githubUrl, level: selectedLevel };
        }
        ...
        document.getElementById('resultStats').innerHTML = \`
          <div class="result-stat">
            <div class="result-stat-value">\${data.stats.originalSize}</div>
            <div class="result-stat-label">Original Size</div>
          </div>
          <div class="result-stat">
            <div class="result-stat-value">\${data.stats.obfuscatedSize}</div>
            <div class="result-stat-label">Obfuscated Size</div>
          </div>
          <div class="result-stat">
            <div class="result-stat-value" style="color:var(--warning)">\${data.stats.increase}</div>
            <div class="result-stat-label">Size Increase</div>
          </div>
          <div class="result-stat">
            <div class="result-stat-value" style="color:var(--success)">\${data.level.toUpperCase()}</div>
            <div class="result-stat-label">Protection</div>
          </div>
        \`;
        ...
        showToast('Failed to obfuscate: ' + err.message, 'error');
      } finally {
        hideLoading();
      }
    }
    ...
    function downloadResult() {
      const blob = new Blob([currentResult], { type: 'application/javascript' });
...
async function handleObfuscate(code, level) {
  ...
  const config = getObfuscationConfig(level);
  
  // In Cloudflare Workers, we need to use a different approach
  // Since javascript-obfuscator may not work directly in Workers,
  // we'll use a lightweight transformation
  ...
  if (level === 'light') {
    ...
    result = result.replace(/\b(?:const|let|var)\s+(\w+)/g, (match, name) => {
      return match.replace(name, '_' + Math.random().toString(36).substring(2, 8));
    });
    result = result.replace(/\b(\w+)\s*=/g, (match, name) => {
      if (['const', 'let', 'var', 'function', 'if', 'for', 'while', 'return'].includes(name)) return match;
      return match.replace(name, '_' + Math.random().toString(36).substring(2, 8));
    });
  }
  ...
  if (level === 'medium' || level === 'heavy') {
    ...
    result = result.replace(/["'`]([^"'`]+)["'`]/g, (match, str) => {
      ...
      return \`(function(){const _='\${encoded}';return Buffer.from(_,'base64').toString()})()\`;
    });
  }
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === 'POST' && path === '/api/obfuscate/github') {
      try {
        ...
        // Simulate GitHub clone (in production, fetch raw files)
        const response = await fetch(body.url.replace('github.com', 'raw.githubusercontent.com').replace('/blob/', '/'));
    ...
    if (method === 'GET' && path === '/api/stats') {
      return jsonResponse({
        uptime: '99.9%',
        levels: 3,
        supportedFormats: ['js', 'mjs', 'cjs'],
        poweredBy: 'CRYSNOVA AI'
      });
    }
